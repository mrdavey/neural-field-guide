...
All 182 released lessons now place one still concept plate after the continuous prose explanation and causal walkthrough, immediately before the interactive scroll story. The complete atlas is 130 generated raster illustrations plus 52 deterministic SVG/HTML diagrams. Each plate is introduced by the first lesson-specific mechanism statement rather than by a caption about the picture. It provides four exact code-native stage labels, a mental model, and a complete text-only explanation containing the stage explanations and important limit. Generated plates are explicitly labeled as illustrations rather than measurements; deterministic diagrams are labeled as illustrative layouts. `docs/LESSON_VISUAL_ATLAS.md` owns the full inventory, style, prompt/provenance, generation, accessibility, and verification contract.
...
Nine deterministic lab modes cover the 46 lessons. Each lab states its complete Change → Observe → Explain → Complete contract before the controls and deterministic readout, so the learner knows which variable to manipulate, which response to inspect, how to explain it, and when the task is complete. Reuse is deliberate when several lessons need the same controllable mechanism. The first World Models lesson uses the `wm-state` surface as a qualitative route-sensitive-map versus vivid-video comparison, deferring signed state arithmetic to the lessons that teach it.
...
All 30 Generative Models lessons use an authored three-case mechanism lab. The learner first sees the question, activity contract, and three case names, then commits a prediction. Case controls, values, meter, and explanations are revealed only after that commitment, so the result cannot answer the prediction in advance. Each lesson separately states what the fixture cannot establish. The shared case-control renderer supplies keyboard/touch buttons, an accessible live readout, and a semantic meter, but lesson data—not animation—owns the result.
...
All 30 Embodied AI lessons use an authored three-case physical-loop lab. The learning question names a task, sensor, frame, state, data, policy, control, authority, timing, or evidence variable. Case names appear before prediction; typed values and the mechanism-specific trace remain behind the same commitment gate. Ordinary buttons, an accessible live region, and a semantic meter preserve keyboard and touch operation; the lesson data owns every result.
...
- Reflowed live-trace headings and stage explanations into rows, replaced 728 duplicated heading/body pairs with short stage names plus explanatory sentences, and kept the visual sticky at desktop widths.
- Restored the complete **Change / Observe / Explain / Complete** activity contract before every lab so instructions are never hidden in an auxiliary disclosure.
