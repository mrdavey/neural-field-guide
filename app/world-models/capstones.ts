...
function project(seed: CapstoneSeed): CapstoneProject {
  return {
    ...
    exemplar: { title: "A complete bounded approach", summary: `The reference pins ${seed.fixture}, compares the target mechanism with ${seed.baseline}, runs ${seed.changedCase}, computes ${seed.metric}, and preserves ${seed.failure} Its conclusion is explicitly limited to the deterministic fixture.`, decisions: ["Committed the threshold before reading final rows.", "Matched the expensive unit and preserved every seed/case.", "Separated a mechanism check from model-quality or real-world evidence."] },
...
const seeds: CapstoneSeed[] = [
  { lessonId: "belief-states-filtering", title: "Build a partially observed tracker", outcome: "Implement a two-state Bayes filter that predicts through actions, updates from noisy observations, and recovers after misleading evidence.", prerequisite: "Belief States and Bayesian Filtering", question: "Does action-aware filtering recover the hidden state more accurately than using the latest observation alone after one misleading sensor reading?", fixture: "a two-state hidden-world sequence with declared transition and observation tables", baseline: "the latest-observation classifier", changedCase: "a held-out trajectory with the same dynamics but a flipped observation at step three", metric: "step accuracy and final posterior mass", failure: "Use an observation likelihood of zero and show how smoothing or an explicit impossible-event policy changes recovery.", source: wmSources.planet },
...
export const worldModelCapstoneEvidencePacks = Object.fromEntries(seeds.map((seed) => [seed.lessonId, {
  ...
  reference: { title: `Complete ${seed.title.toLowerCase()} reference`, sections: [
    ...
    { heading: "2 · Matched comparison", content: `The target mechanism and ${seed.baseline} receive the same cases, horizon, model-transition budget, seed policy, and evaluator. Raw rows remain in the downloadable artifact.` },
    { heading: "3 · Changed case and failure", content: `The transfer test is ${seed.changedCase} The package also preserves this diagnostic failure: ${seed.failure}` },
