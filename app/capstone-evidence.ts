...
export const capstoneEvidencePacks: Record<string, CapstoneEvidencePack> = {
  ...
  "gpt2-from-scratch": {
    ...
    sources: [
      { label: "nanochat speedrun leaderboard record", revision: "README revision 92d63d4; reviewed 17 Jul 2026", url: "https://github.com/karpathy/nanochat/blob/92d63d4/README.md", readFor: "Verify the 1.65-hour entry, DCLM CORE threshold, 8×H100 hardware boundary, and linked training commit rather than treating the headline as a generic runtime." },
      { label: "nanochat 1.65-hour training revision", revision: "training commit a825e63 (14 Mar 2026 leaderboard entry)", url: "https://github.com/karpathy/nanochat/tree/a825e63", readFor: "Inspect the code and `runs/speedrun.sh` revision associated with the leaderboard entry before adapting—not copying—the reference." },
  ...
  "observability-governance": {
    ...
    reference: { title: "Complete source-grounded agent package", sections: [
      ...
      { heading: "Trace schema and evaluation table", content: "OpenTelemetry semantic conventions v1.41.1 marks the GenAI conventions as Status: Development. Pin the convention version and the emitter's migration choice: an existing emitter using conventions at or before v1.36 must not silently change its default, while the latest experimental GenAI conventions require the explicit `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental` opt-in. Then record versioned prompt/model/retriever/reranker/policy IDs; hashed/redacted inputs; retrieved document IDs/scores; claims/citations; proposed/authorized tool calls; latency/tokens/cost; evaluation events; errors; termination and rollback linkage. Evaluation includes retrieval recall, citation entailment, answer coverage, abstention, action validity, attack success, overblocking, tail latency, and cost with uncertainty." },
...
export const capstoneArtifactFiles: Record<string, { label: string; url: string; contents: string[] }> = {
  ...
  "gpt2-from-scratch": { label: "Executable one-block Tiny-GPT correctness fixture · JSON", url: publicPath("capstone-artifacts/tiny-gpt-reference-run.json"), contents: ["dependency-free one-block, one-head, width-3 generator", "five recomputed correctness tests", "fixture training metrics", "checkpoint/resume equivalence", "fixture position ablation—not a 4-layer measured result"] },
