  ...
  "tensors-shapes": {
    ...
    sections: [
      { title: "Why a language model needs tensors at all", paragraphs: [
        ...
        "Suppose two prompts have already been divided into three text positions each. Their integer IDs can be arranged as $[B,T]=[2,3]$: batch $B$ counts prompts and time $T$ counts positions within each prompt. A later embedding lookup—selecting one learned row for each text-piece ID—gives every position $d=4$ numerical features, so the hidden state is $X[B,T,d]=X[2,3,4]$. Naming the axes matters more than memorizing the number of dimensions: $[2,3,4]$ means something different if 2 counts groups rather than prompts.",
        "This numerical lesson comes before the architecture so later diagrams are readable instead of magical. Lesson 6 explains how text becomes text-piece IDs; Lesson 7 turns each ID into a feature vector; attention later mixes information among positions; an MLP transforms features within each position; and the output head produces one score per possible next piece. These components repeatedly project $[B,T,d]$ states. The remaining Foundations lessons explain how the scores become a loss, how the loss produces gradients, and how an optimizer updates the learned matrices. Tensors are the shared language connecting that whole path, not a detached mathematics detour.",
      ] },
      { title: "Matrix multiplication applies one learned transformation everywhere", paragraphs: [
        "Many operations in the LLM architecture introduced in later lessons keep the prompt and position axes while changing the features carried at each position. A linear layer stores a learned weight $W[d,d_{out}]$ and computes $$[B,T,d] \\times [d,d_{out}] \\to [B,T,d_{out}].$$ The matching $d$ labels identify the feature axis that is combined; $B$ and $T$ are not combined, so they survive. The new $d_{out}$ axis names the output feature width.",
        "Each output feature is a dot product between one position's input vector and one learned column of the weight matrix. If a position has four input features and the matrix produces six outputs, the layer performs six four-term dot products for that position. The same matrix is reused at every position: attention projects features before mixing information among positions; an MLP projects features to transform them within one position; and the output head projects features into vocabulary scores. The arithmetic is the same, while the learned weights and purpose differ.",
      ] },
      { title: "A runnable shape can still encode the wrong idea", paragraphs: [
        "A bias is one learned offset per output feature. Broadcasting can add that bias $[d_{out}]$ to every prompt-position in $[B,T,d_{out}]$ without making explicit copies. It can also repeat one position-specific number across all features at that position. Broadcasting is easy to misuse: a technically compatible axis can broadcast while representing the wrong concept. Good practice is to annotate axes, assert shapes at boundaries, and test with unequal dimensions so accidental swaps cannot hide.",
      ] },
    ],
    walkthrough: [
      { title: "Start from the response-building job", body: "Treat the prompt as already divided into positions. The model needs one changing numerical state for every visible position before it can score what comes next: $X[2,3,4]$ means two prompts, three text positions per prompt, and four features per position.", checkpoint: "This lesson takes the text division as an input; Lesson 6 explains how a tokenizer chooses it. Changing $B$ groups more prompts rather than adding positions or features." },
  ...
  "probability-softmax": {
    ...
    sections: [
      { title: "Scores first, probabilities second", paragraphs: [
        "The previous lesson ended with hidden states $X[B,T,d]$. A final learned projection $W[d,V]$ maps every prompt-position to logits $[B,T,V]$, where vocabulary width $V$ lists one raw score for each possible next text piece. A language model therefore does not directly output percentages. Logits can be negative, positive, large, or small; only their differences matter. Softmax runs over the final $V$ axis at each prompt-position, exponentiating relative scores and dividing by their total so the probabilities are non-negative and sum to one.",
