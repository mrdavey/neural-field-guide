...
export const lessons: Lesson[] = [
  ...
  },
  {
    ...
    deep: "GPT-2 is a decoder-only, pre-LayerNorm Transformer using learned absolute positional embeddings, masked multi-head self-attention, GELU MLPs, residual connections, a final LayerNorm, and a vocabulary projection. A forward pass is token+position lookup → repeated LN→masked MHA→add→LN→MLP→add blocks → final LN → logits. For next-token loss, logits at positions 0…T−2 align with labels 1…T−1. A minimal loop performs zero_grad, forward, cross-entropy, backward, optimizer step; train/eval mode controls dropout. During generation, a KV cache can reuse earlier attention keys/values rather than recomputing the whole prefix.",
    ...
    example: "With B=4, T=128, V=50,257, logits are [4,128,50,257]—about 25.7 million values. Training flattens shifted logits [B(T−1),V] against next-token labels [B(T−1)]. Pseudocode: zero_grad(); logits=model(x); loss=CE(logits[:,:-1], x[:,1:]); loss.backward(); optimizer.step().",
    ...
    quiz: { question: "If input IDs have shape [B,T], which labels align with logits at positions 0 through T−2?", options: ["The same IDs x[:, :-1]", "The next IDs x[:, 1:]", "Only the final token", "Random vocabulary IDs"], answer: 1, explanation: "Causal training shifts targets by one: each position predicts the token immediately to its right." }, lab: "gpt",
  ...
  },
  {
    ...
    deep: "The April 2024 Llama 3 release covered 8B/70B models trained on 15T+ tokens, a 128K-token vocabulary, GQA, and 8,192-token training sequences. The July 2024 Llama 3.1 release added the 405B model and 128K context across the herd—do not project those later facts backward. Compared with Llama 2, the larger tokenizer improves text compression, the much larger curated mixture increases data exposure, and GQA reduces inference KV-cache cost. These are versioned engineering claims, not a single timeless ‘Llama 3’ specification.",
    ...
    example: "Suppose 32 query heads use 8 key/value heads with head dimension 128. Standard MHA caches 32 K and 32 V heads per token; GQA caches 8+8 instead of 32+32—4× fewer KV elements for those projections. The query-side head count remains 32.",
    misconception: "“Open weights” does not mean every training datum, pipeline detail, or unrestricted usage right is open.",
    quiz: { question: "Which claim belongs to the initial April 2024 Llama 3 8B/70B release rather than the later 3.1 herd?", options: ["A 405B model", "128K context across the herd", "8,192-token training sequences and 15T+ training tokens", "No grouped-query attention"], answer: 2, explanation: "The initial release reported 8B/70B, 15T+ tokens, GQA, and 8,192-token sequences; 405B and 128K context are Llama 3.1 facts." }, prerequisites: ["pretraining-evaluation"], sources: [{ label: "Meta: Introducing Meta Llama 3", url: "https://ai.meta.com/blog/meta-llama-3/" }, { label: "Meta: The Llama 3 Herd of Models", url: "https://ai.meta.com/research/publications/the-llama-3-herd-of-models/" }]
  },
  {
  ...
  },
  {
    ...
    deep: "AI2’s Tülu 3 recipe proceeds through prompt curation/synthesis, supervised fine-tuning, preference tuning with on-policy response data and length-normalized DPO, then Reinforcement Learning with Verifiable Rewards (RLVR). RLVR uses a programmatic verifier for outcomes instead of a learned reward model: exact-answer math, unit tests, or constrained instruction checks can yield a binary signal. Length normalization matters because preference optimization can otherwise reward longer responses through accumulated log-probabilities. Open-ended helpfulness still needs human or model judgments, and a buggy or gameable verifier can confidently optimize the wrong behavior.",
    ...
    keyIdeas: ["Recipe: curate prompts → SFT → preference tuning → RLVR", "RLVR uses checkable outcomes rather than a learned reward model", "On-policy data and length normalization address preference-distribution and length biases"],
    example: "For ‘17×6’, a verifier can parse the final answer and return 1 only for 102. For ‘use exactly three bullets’, a structural checker can verify the constraint. But a parser that accepts any response containing ‘102’ creates false positives—illustrating why verifier quality defines RLVR quality.",
    ...
    quiz: { question: "What distinguishes RLVR from classic reward-model RLHF?", options: ["RLVR has no rewards", "RLVR uses a programmatic outcome verifier rather than a learned preference reward model", "RLVR is tokenization", "RLVR requires subjective style only"], answer: 1, explanation: "RLVR is strongest where correctness or constraints can be checked, but it still inherits failures from incomplete or exploitable verifiers." }, lab: "preference", prerequisites: ["tools-safety"], sources: [{ label: "Ai2: Tülu 3", url: "https://allenai.org/tulu" }, { label: "Ai2: Tülu 3 technical overview", url: "https://allenai.org/blog/tulu-3-technical" }]
  },
  {
