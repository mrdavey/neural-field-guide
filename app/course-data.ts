...
export type Lesson = {
  ...
  quiz: Quiz;
  lab?: "orientation" | "tokens" | "vectors" | "positions" | "attention" | "prediction" | "scaling" | "optimizer" | "preference" | "lora" | "moe" | "distillation" | "rl" | "block" | "gpt" | "pipeline" | "dataPipeline" | "olmoFlow" | "posttrainingMap" | "sftMasking" | "rlhfPipeline" | "tuluRecipe" | "objectives" | "systems" | "evaluation" | "tensors" | "softmax" | "gradient" | "decoding" | "kvcache" | "quantization" | "serving" | "testtime" | "context" | "rag" | "agents" | "evaldesign" | "security" | "observability" | "multimodal" | "interpretability" | "wm-state" | "wm-belief" | "wm-latent" | "wm-rollout" | "wm-planner" | "wm-video" | "wm-uncertainty" | "wm-safety" | "wm-evaluation" | "research";
...
const lessonDefinitions: Lesson[] = [
  {
    ...
    deep: "Treat the prompt as already divided into text positions for now; Lesson 6 explains how that division is chosen. Position IDs can be stored as $[B,T]$, where $B$ is the number of prompts processed together and $T$ is the number of positions per prompt. An embedding lookup later produces hidden states $X[B,T,d]$, with $d$ numerical features per position. A learned weight $W[d,d_{out}]$ computes $Y=XW+b$: $$[B,T,d] \\times [d,d_{out}] \\to [B,T,d_{out}].$$ This same shape pattern appears in attention projections, MLP layers, and the vocabulary output head. Broadcasting reuses a smaller tensor across compatible axes—useful for bias and masks, but dangerous when dimensions fit while their meanings do not.",
    ...
    keyIdeas: ["LLMs carry batches of text positions as named numerical axes", "Matrix multiplication applies the same learned feature transformation at every position", "A shape can be compatible while its axis meaning is wrong"],
  ...
  },
  {
    ...
    keyIdeas: ["A prompt supplies the task and context for a response built piece by piece", "The model uses learned language patterns to choose each next piece", "Useful-sounding output still needs the right kind of checking"],
  ...
  },
  {
    ...
    quiz: { question: "What would unmasked, content-only self-attention lack without positional information?", options: ["A vocabulary", "A reliable representation of token order", "Floating-point numbers", "Multiple heads"], answer: 1, explanation: "Unmasked content attention is permutation-equivariant. A causal mask separately adds directional access, while a varying position signal supplies richer index or distance geometry." },
  ...
  },
  {
    ...
    misconception: "nanochat is not merely GPT-2 running on newer GPUs. The README at revision `92d63d4` records a 1.65-hour DCLM CORE capability-threshold entry whose training commit is `a825e63`, run on 8×H100 GPUs. It is not a copied 1.5B checkpoint, a laptop runtime, or a frontier assistant.",
  ...
  },
  {
    ...
    quiz: { question: "Why can a smaller post-training dataset matter so much?", options: ["It changes the tokenizer", "It is concentrated on high-leverage behaviors and response policy", "It retrains every fact", "It removes pre-training"], answer: 1, explanation: "Curated examples directly target how existing capabilities should be expressed." }, lab: "posttrainingMap", prerequisites: ["instruction-tuning-rlhf"]
  },
  {
    ...
    quiz: { question: "Why mask user tokens in many SFT recipes?", options: ["To hide prompts from the model", "To focus optimization on producing the assistant response", "To reduce vocabulary", "To add positional encoding"], answer: 1, explanation: "The prompt remains input context, while loss is concentrated on the output behavior being demonstrated." }, lab: "sftMasking", prerequisites: ["posttraining-overview"]
  },
  {
  ...
  },
  {
    ...
    lab: "rlhfPipeline", prerequisites: ["rl-fundamentals", "preference-optimization"]
  },
  {
  ...
  },
  {
    ...
    keyIdeas: ["Time-to-first-token and inter-token latency are distinct", "Continuous batching reduces padding and idle slots", "Admission and per-class limits protect latency under overload"],
    ...
    lab: "serving", prerequisites: ["quantization-memory"], sources: [{ label: "Orca continuous batching", url: "https://www.usenix.org/conference/osdi22/presentation/yu" }]
  },
  {
