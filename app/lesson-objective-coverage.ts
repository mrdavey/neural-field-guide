...
const coveragePlans: Record<string, ObjectiveCoveragePlan[]> = {
  ...
  "tensors-shapes": [
    cover(0, "An LLM constructs text piece by piece, but the learned program operates on numbers. Tensors keep the numerical state for many prompts and text positions organized so the same learned operations can process them together.", [paragraph(0, 0), paragraph(0, 1), paragraph(0, 2), walkthrough(0), walkthrough(2)], [exampleSetup(), exampleStep(1), exampleStep(2)], [checkpoint(0), checkpoint(2)], "Two already-divided prompts each contain three text positions, and every position carries four features. State the hidden-state shape, explain every axis, and say why this tensor is useful to an LLM.", [literal("The hidden state is $X[2,3,4]$: 2 prompts, 3 text positions per prompt, and 4 numerical features per position. The tensor lets one learned operation process all six current positions while preserving which prompt and position each feature vector belongs to; later attention, MLP, and output projections reuse this layout.")], [literal("Return to ‘Why a language model needs tensors at all.’ First write [prompt, position, feature] under the three numbers. Then trace one position through the learned projection and name the later LLM components that reuse the same axes.")]),
    cover(1, "Matrix multiplication contracts one matching feature axis, applies the same learned transformation at every text position, and preserves the prompt and position axes.", [paragraph(1, 0), paragraph(1, 1), walkthrough(1), walkthrough(2)], [exampleStep(0), exampleStep(1), exampleStep(2)], [checkpoint(1), checkpoint(2)], "Predict the output shape of X[2,5,4]W[4,7] and calculate how many terms contribute to one output feature.", [paragraph(1, 0), paragraph(1, 1)], [walkthrough(1)]),
    cover(2, "Broadcasting reuses a smaller tensor across compatible axes, which is useful only when the aligned axes have the intended meaning.", [paragraph(2, 0)], [literal("For X[2,3,6], adding bias[6] deliberately reuses the six feature values at all six batch-position locations. A tensor shaped [3,1] also broadcasts, but it repeats one value across all six features at each text position. The shapes run even when the second meaning is not the intended operation.")], [literal("Broadcast compatibility proves only that dimensions can be expanded. It cannot prove that the expanded axis represents the intended prompt, text-position, or feature role.")], "A bias [6] broadcasts over X[2,3,6]. Explain why that is valid and give one semantically wrong broadcast that could still run.", [paragraph(2, 0)], [paragraph(2, 0)]),
  ],
  "probability-softmax": [
  ...
  "positional-encoding": [
    cover(0, "Without a varying position signal, unmasked content-only self-attention can compare tokens but is permutation-equivariant: reordering inputs merely reorders outputs. A causal mask is a separate structural signal that adds earlier-versus-later legal access, not a reusable absolute-index or pairwise-distance geometry.", [paragraph(0, 0), walkthrough(0)], [exampleSetup(), exampleStep(0), exampleStep(1), exampleStep(2)], [checkpoint(0)], "Reorder the same token vectors from [dog, chased, cat] to [cat, chased, dog]. Contrast what unmasked content-only attention, a causal mask, and a varying positional signal can distinguish.", [exampleStep(0), exampleStep(1), exampleStep(2), practiceAnswer()], [paragraph(0, 0)]),
  ...
  attention: [
    cover(0, "Attention lets each token query other visible positions, score their keys, and combine their value vectors into an information update. In a causal decoder, the query must be at or after every key used; the worked trace therefore queries the later token ‘sturdy’ rather than letting an earlier pronoun read its future adjective.", [paragraph(0, 0), paragraph(0, 1), walkthrough(0), walkthrough(1), walkthrough(2)], [exampleSetup(), exampleStep(0), exampleStep(1), exampleStep(2)], [checkpoint(0)], "At the legal query ‘sturdy’, name the earlier visible keys, then trace Q,K,V shaped [B,h,T,d_h] through score shape, softmax axis, weighted-value shape, and final residual-width output.", [exampleSetup(), exampleStep(0), paragraph(0, 0), paragraph(0, 1), walkthrough(2)], [walkthrough(0), walkthrough(1), walkthrough(2)]),
  ...
  "learning-to-predict": [
    cover(0, "Causal training shifts a sequence so every scored input is paired with the next token in the same document. In packed data, final content still predicts EOS, while the artificial EOS→BOS transition between documents is omitted.", [paragraph(0, 0), paragraph(0, 2), walkthrough(0)], [exampleSetup(), exampleStep(0), exampleStep(1), exampleStep(2)], [checkpoint(0)], "For [BOS, red, fox, EOS, BOS, blue, moon, EOS], list the within-document scored pairs, retain both final-content→EOS targets, and identify the cross-document pair that must be omitted.", [paragraph(0, 2), checkpoint(0)], [walkthrough(0)]),
    cover(1, "Teacher forcing supplies every true prefix, allowing legal token losses to be computed in parallel while causal and segment-aware masks block future and cross-document leakage. Resetting position IDs alone does not create document isolation.", [paragraph(0, 1), paragraph(0, 2), walkthrough(1)], [exampleStep(0), exampleStep(1), exampleStep(2)], [checkpoint(1)], "For two documents packed into one row, explain how teacher forcing remains parallel and causal, then specify segment IDs plus block-diagonal attention (or separate sequence execution) so document 2 cannot read document 1.", [paragraph(0, 1), paragraph(0, 2), checkpoint(1)], [walkthrough(1)]),
  ...
  "preference-optimization": [
    ...
    cover(1, "DPO raises the chosen-versus-rejected log-probability gap relative to a fixed reference. In the displayed logistic loss, larger beta sharpens the response to a fixed log-ratio margin; in the constrained-policy derivation, larger beta means stronger KL/reference restraint for a fixed reward gap. The fixed quantity and convention must be stated before comparing beta values.", [paragraph(0, 1), walkthrough(2)], [exampleStep(0), exampleStep(1)], [checkpoint(2)], "For one chosen/rejected pair, describe the policy gap and reference gap, then state both effects of increasing beta: fixed log-ratio margin in the displayed loss versus fixed reward gap in the constrained-policy derivation.", [paragraph(0, 1)], [walkthrough(2)]),
...
export const objectiveCoverageRemediations: Record<string, ObjectiveCoverageRemediation> = {
  ...
  "tensors-shapes#3": {
    ...
    check: {
      expected: "For $X[2,3,6]$, bias $[6]$ aligns with the final feature axis and repeats the same six feature offsets across both prompts and all three positions. Offsets $[3,1]$ also expand to $[2,3,6]$, but they encode one scalar per position and repeat it across all six features and both prompts. If the intended operation was one independent offset per feature, the second broadcast is semantically wrong even though it runs.",
      retry: "Label the target axes $[batch,position,feature]$, align $[6]$ and $[3,1]$ from the right, and state which values repeat along each missing axis. Then choose unequal position and feature sizes so an accidental swap cannot hide.",
    },
  },
