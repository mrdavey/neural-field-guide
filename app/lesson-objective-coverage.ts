...
const coveragePlans: Record<string, ObjectiveCoveragePlan[]> = {
  introduction: [
    cover(0, "An LLM repeatedly predicts a probability distribution for the next token from the tokens already in its context.", [paragraph(0, 0), walkthrough(1), walkthrough(2)], [exampleSetup(), exampleStep(0), exampleStep(1), exampleStep(2)], [paragraph(0, 1), checkpoint(1)], "For the prompt ‘Water freezes at’, name the model output, the decoder's action, and what becomes the next input.", [walkthrough(1), walkthrough(2), exampleStep(0)], [paragraph(0, 0)]),
    cover(1, "Training and post-training change parameters; inference uses those fixed parameters, while decoding chooses tokens from their output distribution.", [paragraph(1, 0)], [exampleStep(0), exampleStep(1)], [checkpoint(2)], "Temperature changes two answers from the same prompt. Which stage changed, and which stages definitely did not run again?", [practiceAnswer()], [paragraph(1, 0)]),
    cover(2, "Fluent text can be wrong because predicting plausible text is different from checking a claim against reliable evidence.", [paragraph(0, 1), paragraph(1, 1)], [exampleStep(0), exampleResult()], [checkpoint(1), paragraph(1, 1)], "A polished answer gives today's exchange rate without a source. Explain why fluency is insufficient and what the surrounding system must add.", [paragraph(1, 1)], [paragraph(0, 1)]),
  ],
  "tensors-shapes": [
...
export const objectiveCoverageRemediations: Record<string, ObjectiveCoverageRemediation> = {
  "introduction#2": {
    workedExample: "One staged case: during pre-training, ‘The capital of France is Paris’ contributes next-token loss and changes parameters. During post-training, an assistant demonstration such as user: ‘Name France’s capital’ → assistant: ‘Paris’ changes parameters again to favor the dialogue behavior. During inference, those parameters stay fixed while the prompt produces logits. During decoding, greedy selection chooses the highest-probability next token ‘Paris’; no training occurs during that choice.",
  },
  "tensors-shapes#2": {
    check: {
